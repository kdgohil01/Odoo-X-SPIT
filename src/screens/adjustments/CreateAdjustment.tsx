...
export function CreateAdjustment() {
  ...
  const updateLine = (index: number, field: 'productId' | 'recordedQuantity' | 'physicalQuantity', value: any) => {
    setLines(lines.map((line, i) => {
      if (i === index) {
        ...
        // Auto-update recorded quantity when product is selected
        if (field === 'productId' && value && formData.warehouseId) {
          const currentStock = getStockByProductAndWarehouse(value, formData.warehouseId);
          updatedLine.recordedQuantity = currentStock;
        }
  ...
  return (
    <div>
      <Header title="Create Stock Adjustment" />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        ...
                        // Auto-update recorded quantities for all lines when warehouse changes
                        setLines(lines.map(line => {
                          if (line.productId) {
                            const currentStock = getStockByProductAndWarehouse(line.productId, value);
                            return { ...line, recordedQuantity: currentStock };
                          }
                          return line;
                        }));
                      }}
                    ...
                    return (
                      <div key={index} className="flex gap-4 items-end">
                        <div className="flex-1 space-y-2">
                          ...
                          >
                            ...
                            <SelectContent>
                              {products.map(product => {
                                const currentStock = formData.warehouseId ? getStockByProductAndWarehouse(product.id, formData.warehouseId) : 0;
                                return (
                                  <SelectItem key={product.id} value={product.id}>
                                    <div className="flex justify-between items-center w-full">
                                      <span>{product.name} ({product.sku})</span>
                                      <span className="text-xs text-muted-foreground ml-2">Stock: {currentStock}</span>
                                    </div>
                                  </SelectItem>
                                );
                              })}
                          ...
                          {line.productId && formData.warehouseId && (
                            <div className="text-xs text-muted-foreground">
                              Current stock: {getStockByProductAndWarehouse(line.productId, formData.warehouseId)} {products.find(p => p.id === line.productId)?.uom}
                            </div>
                          )}
                        </div>
